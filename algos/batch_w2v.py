#%matplotlib inline
# This file is modification of udacity word2vec assignment 5 to support twitter data.
...
# foreach file read file names.
if os.path.exists(dirpath):
  root_path = os.getcwd()
  for input_filename in os.listdir(dirpath):
    if not input_filename.startswith('.'):
      filename_wo_ext = os.path.splitext(os.path.basename(input_filename))[0]
      with zipfile.ZipFile(dirpath+input_filename) as f:
        text = f.read(f.namelist()[0])
        big_regex = re.compile('|'.join(map(re.escape, supp_list)))
        fix_text = big_regex.sub("stjohnswort", text)
  wordslist.extend(tf.compat.as_str(fix_text).split())
  # Combine words list.
...
#num_points = 75000 ## correct entry
#num_points = 500 ## ravish added
num_points = 98 ## ricardo added

words = [reverse_dictionary[i] for i in range(1, num_points+1)]

write_vector_space_to_file(final_embeddings, reverse_dictionary.values())
# Do PCA for plot
tsne = TSNE(perplexity=30, n_components=2, init='pca', n_iter=5000)
two_d_embeddings = tsne.fit_transform(final_embeddings[1:num_points+1, :])
def plot(embeddings, labels):
# def plot(embeddings, labels, words_to_plot):
  assert embeddings.shape[0] >= len(labels), 'More labels than embeddings'
  pylab.figure(figsize=(15,15))  # in inches
  for i, label in enumerate(labels):
...
#plot(two_d_embeddings, words, words_to_plot) # rc 2
